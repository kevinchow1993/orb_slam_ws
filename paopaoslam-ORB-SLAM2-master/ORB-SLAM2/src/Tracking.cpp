...
void Tracking::CreateInitialMapMonocular()
{
    ...
    // 步骤5：BA优化 3D-2D 最小化重投影误差 e = (u,v) - project(Tcw*Pw)，迭代20次。得到优化后的结果，把优化后的位姿传递给帧参与优化的帧，更新优化后的空间点。
    Optimizer::GlobalBundleAdjustemnt(mpMap,20);

    // Set median depth to 1
    // 步骤6：!!!将MapPoints的中值深度归一化到1，并归一化两帧之间变换 将MapPoints的中值深度归一化到1，并归一化两帧之间变换， x/z ，y/z， 将z归一化到1 。
    // 评估关键帧场景深度，q=2表示中值
    ...
    // 这部分和SteroInitialization()相似 初始关键帧和当前帧插入到地图   
    mpLocalMapper->InsertKeyFrame(pKFini);
...
bool Tracking::TrackReferenceKeyFrame()
{
    ...
    //D-2D 最小化重投影误差 e = (u,v) - project(Tcw*Pw) ，只优化Frame的Tcw，不优化MapPoints的坐标。开始优化，
    //总共优化四次，每次优化后，将观测分为outlier和inlier，outlier不参与下次优化
    ...
    while(nCandidates>0 && !bMatch)
    {
        for(int i=0; i<nKFs; i++)
        {
            ...
            if(!Tcw.empty())
            {
                ...
                //如果，内点的数量少于10个，就跳过本次循环，如果内点(10<nGood<50)，则通过投影的方式对之前当前帧和候选帧未匹配的点进行匹配
                if(nGood<10)
                    continue;
